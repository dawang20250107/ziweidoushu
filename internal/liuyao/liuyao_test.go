...
// TestJudge 确定性断语:随起卦即出,含综断/等级/用神/应期,逐条非空。
func TestJudge(t *testing.T) {
	r, err := ByTosses([]int{2, 3, 1, 3, 2, 2}, time.Date(2026, 1, 15, 10, 0, 0, 0, time.UTC), "求财能否得手")
	if err != nil {
		t.Fatal(err)
	}
	j := r.Judgment
	...
	if j.Conclusion == "" || j.Level == "" || j.YongShen == "" || j.YingQi == "" {
		t.Errorf("断语字段缺失: %+v", j)
	}
	...
	// 用神妻财,断语应引用妻财。
	if !strings.Contains(j.YongShen, "妻财") {
		t.Errorf("求财用神应为妻财: %s", j.YongShen)
	}
}
