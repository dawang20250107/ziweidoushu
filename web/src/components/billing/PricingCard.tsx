...
export function PricingCard({
  ...
  return (
    <div
      className={[
        "relative flex flex-col rounded-[10px] bg-bg-raised p-6",
        featured
    ...
    >
      {featured && (
        <span className="absolute -top-3 left-6 inline-flex items-center rounded-[2px] bg-gold px-2 py-0.5 text-[11px] font-semibold tracking-[0.08em] text-[#161206]">
          最划算
        </span>
      )}

      <p className="text-[12px] tracking-[0.16em] text-gold">{eyebrow}</p>
      <h3 className="mt-1.5 font-display text-lg font-semibold text-ink">{product.title}</h3>
      ...
      <ul className="mt-5 flex flex-1 flex-col gap-2.5">
        {benefits.map((b) => (
          <li key={b} className="flex items-start gap-2 text-[14px] text-ink-secondary">
            <Check />
            <span>{b}</span>
          </li>
        ))}
      ...
      <button
        ...
        className={[
          "mt-6 w-full rounded-[6px] px-6 py-3 text-[15px] font-medium transition-colors disabled:opacity-40",
          featured
            ? "bg-gold text-[#161206] hover:bg-gold-bright"
